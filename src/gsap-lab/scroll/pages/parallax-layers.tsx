...
export function ParallaxLayersPage() {
  ...
  return (
    <div ref={container}>
      <ScrollDemoShell title="패럴랙스 레이어" summary="겹친 레이어가 스크롤에 서로 다른 양만큼 밀린다 (y = innerHeight × 비율, ease: none)">
        <section className="parallax-stage relative h-[240vh]">
          ...
          <div className="sticky top-0 flex h-screen items-end justify-center overflow-hidden">
            ...
            <p className="relative z-10 mb-10 rounded-full bg-black/40 px-4 py-2 text-sm text-white">스크롤 — 뒤 레이어가 더 빨리 올라갑니다</p>
          </div>
        </section>

        <section className="mx-auto max-w-2xl px-6 py-32 text-center text-white/70">
          <p>배경 레이어는 뷰포트 높이의 절반 이상 밀려 올라가고, 전경 레이어는 거의 제자리입니다. 되감으면 역방향으로 움직입니다.</p>
        </section>
      </ScrollDemoShell>
    </div>
  );
