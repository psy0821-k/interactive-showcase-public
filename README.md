...
> three.js·GSAP를 예제 수준에서 실무 수준으로 끌어올리려고 만든, 직접 상호작용하는 3D 웹 기법 쇼케이스 모음입니다.
...
three.js와 GSAP를 대략적으로만 다룰 줄 알았습니다. 예제는 따라 만들지만, 실무에서 "왜 그림자가 갑자기 사라지는지", "모바일에서 스크롤이 왜 막히는지" 같은 문제를 만나면 원인을 짚지 못하는 수준이었습니다.

이 간극을 스스로 좁히려고, AI로 씬 초안을 빠르게 만들고 그것을 성능·접근성·구조 측면에서 직접 다듬어가며 39개의 라이브 쇼케이스를 만들었습니다. 각 쇼케이스에는 어떤 skill을 어떻게 썼는지와 같은 씬을 재현할 요청 예시를 담았고, 실무에서 가장 자주 하는 작업 3개는 "AI 초안이 이렇게 틀렸다 → 이렇게 고쳤다"를 별도로 기록했습니다.
...
| 축              | 내용                                                                                                                 |
| --------------- | -------------------------------------------------------------------------------------------------------------------- |
| 문제 진단       | AI 초안이 자주 틀리는 지점(리렌더 폭증, 카메라 소유권 충돌, 컬러스페이스, 기기 감지)을 짚어 재구조화 — 아래 대표 3개 |
