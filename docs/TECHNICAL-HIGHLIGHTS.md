...
3D 웹 기법을 **직접 상호작용하는 라이브 쇼케이스**로 만들면서, 개발 중
실제로 부딪힌 함정을 §4에 정리했다. 각 쇼케이스의 `meta.ts`에는 어떤 skill을
어떻게 썼는지와 재현용 요청 예시를 담고, 실무 빈도가 높은 3개는 "AI 초안을
어떻게 고쳤는지"를 별도로 기록했다.

- **쇼케이스**: `src/showcases/{category}/{name}/` × 39 (8개 카테고리)
...
쇼케이스를 만들며 부딪힌 "조용히 깨지는 지점"을 모았다. "왜 안 되지"에 쓰는
시간이 3D 웹에서 가장 크기 때문이다.
